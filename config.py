#!/bin/env python3

symlinks = [
	(sdir + '/Controller/ReferencesController.php', cakedir + '/Controller/ReferencesController.php'),
	(sdir + '/Controller/ReferencePicturesController.php', cakedir + '/Controller/ReferencePicturesController.php'),
	(sdir + '/Model/Reference.php', cakedir + '/Model/Reference.php'),
	(sdir + '/Model/ReferencePicture.php', cakedir + '/Model/ReferencePicture.php'),
	(sdir + '/View/References/add.ctp', cakedir + '/View/References/add.ctp'),
	(sdir + '/View/References/index.ctp', cakedir + '/View/References/index.ctp'),
	(sdir + '/js/reference.js', cakedir + '/webroot/js/reference.js'),
]
